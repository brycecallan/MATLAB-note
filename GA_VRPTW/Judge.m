...
% 输出：flag                0表示违反约束，1表示满足全部约束
    flag = 1;                          % 假设满足约束
    NV = size(VC,1);                   % 车辆使用数目
    init_v = vehicle_load(VC,demands); % 计算每辆车的装载量
    bsv = begin_s_v(VC,a,s,dist);      % 计算每辆车配送路线上在各个点开始服务的时间，还计算返回集配中心时间
    violate_INTW = Judge_TW(VC,bsv,b,L); % 判断是否违背时间窗约束，0代表不违背，1代表违背
